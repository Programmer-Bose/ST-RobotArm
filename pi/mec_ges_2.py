...
class MecanumGestureController(ctk.CTk):
    def __init__(self):
        ...
        self.geometry("1100x750")  # Increased width slightly for the new button
        ...
        # [SCAN BUTTON]
        self.btn_scan = ctk.CTkButton(self.top_frame, text="Scan for 'RS'", command=self.start_scan, width=100, fg_color="purple")
        self.btn_scan.pack(side="left", padx=10)

        self.lbl_ip = ctk.CTkLabel(self.top_frame, text="IP:")
        self.lbl_ip.pack(side="left", padx=5)
        
        self.entry_ip = ctk.CTkEntry(self.top_frame, width=130)
    ...
    # --- SCANNING LOGIC ---
    def start_scan(self):
        """Starts the scanning process in a separate thread to prevent UI freezing"""
    ...
    def perform_scan(self):
        ...
        # METHOD 1: Direct Hostname Resolution (Fastest)
        try:
            found_ip = socket.gethostbyname(f"{target_name}.local")
            print(f"✅ Found via Hostname: {found_ip}")
        except:
            pass

        # METHOD 2: mDNS Service Discovery (If Method 1 fails)
        if not found_ip:
            zeroconf = Zeroconf()
            listener = MyListener(target_name)
            browser = ServiceBrowser(zeroconf, "_http._tcp.local.", listener)
            
            # Wait 3 seconds for discovery
            time.sleep(3)
            
            if listener.found_ip:
                found_ip = listener.found_ip
                print(f"✅ Found via mDNS Service: {found_ip}")
            
            zeroconf.close()

        # Update UI (Must be done on main thread logic, but Tkinter is thread-safe enough for variables usually)
        # We use a helper to update the UI safely
        self.after(0, lambda: self.finish_scan(found_ip))

    def finish_scan(self, ip):
        self.btn_scan.configure(text="Scan for 'RS'", state="normal")
        if ip:
            self.entry_ip.delete(0, "end")
            self.entry_ip.insert(0, ip)
            self.lbl_status.configure(text=f"Found: {ip}", text_color="orange")
        else:
            self.lbl_status.configure(text="Device 'RS' Not Found", text_color="red")
...
# Helper Class for mDNS Discovery
class MyListener:
    ...
    def add_service(self, zeroconf, type, name):
        if self.target_name in name:
            ...
            if info:
                # Convert bytes address to string (e.g. 192.168.1.10)
