...
class DRLDataCollector(ctk.CTk):
    def __init__(self):
        ...
        self.title("DRL Data Collector V7 (Episode Support)")
        ...
        self.first_record_flag = True # Determines if we are at start of episode
    ...
    def init_csv(self):
        headers = ["timestamp", "img_filename", "episode_start"] # Added flag
    ...
    def create_ui(self):
        ...
        ctk.CTkLabel(f_top, text="Speed:").pack(side="left", padx=10)
        self.ent_speed = ctk.CTkEntry(f_top, width=60); self.ent_speed.insert(0, str(DEFAULT_SPEED)); self.ent_speed.pack(side="left")
        ctk.CTkButton(f_top, text="Set", width=50, command=self.set_speed).pack(side="left", padx=5)
    ...
    def end_episode(self):
        """Resets the recording state to start fresh."""
        self.first_record_flag = True
        # If auto-recording is running, you might want to stop it, 
        # OR keep it running but mark the next frame as start. 
        # Here we just mark the flag.
        self.lbl_status.configure(text="Episode Ended. Next frame will be new start.", text_color="orange")
        print("--- EPISODE ENDED ---")
    ...
    def record_snapshot(self):
        ...
        if self.first_record_flag:
            ...
            episode_start_val = 1 # Mark this row as start of episode
        else:
        ...
        # CSV: Added episode_start column at index 2
